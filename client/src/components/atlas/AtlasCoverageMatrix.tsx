...
import { Brain, RefreshCw, Target, CheckCircle2, Circle, AlertCircle, LayoutGrid, List } from "lucide-react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export default function AtlasCoverageMatrix() {
  ...
  const fetchCoverage = async (operationId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Failed to fetch ATLAS coverage data");
      }
    } catch (error) {
      // Error already shown via toast
    } finally {
  ...
  const renderHeatmapView = () => {
    if (tactics.length === 0 || allTechniques.length === 0) {
      return (
        <div className="text-center py-12 bg-card rounded-lg border border-border">
          ...
          <p className="text-muted-foreground font-medium">Loading ATLAS matrix data...</p>
        </div>
      );
    }
    ...
    tactics.forEach((tactic) => {
      techniquesByTactic[tactic.atlasId] = allTechniques.filter((t) =>
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <div className="min-w-max">
            ...
            <div className="flex border-b border-border bg-purple-50 dark:bg-purple-950/30">
            ...
            <div className="divide-y divide-gray-200">
              {allTechniques.slice(0, 50).map((technique) => {
                const techniqueCoverage = coverage.filter(
                  (c) => c.technique.atlasId === technique.atlasId
                );
                  ...
                  <div key={technique.id} className="flex hover:bg-secondary">
                    ...
                    {tactics.map((tactic) => {
                      ...
                      const mapping = techniqueCoverage.find(
                        (c) => c.tactic?.atlasId === tactic.atlasId
                      );
                                ...
                                <TooltipContent>
                                  <div className="text-sm">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                      {technique.atlasId} → {tactic.atlasId}
                                    </p>
                                    {mapping ? (
                                      <>
                                        <p className="mt-1">
        ...
        {allTechniques.length > 50 && (
          <div className="p-4 bg-purple-50 dark:bg-purple-950/30 border-t border-border text-center text-sm text-muted-foreground">
    ...
    return (
      <div className="text-center py-12 bg-card rounded-lg border border-border">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Create an operation first to track ATLAS technique coverage
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Track ATLAS ML/AI technique coverage for operations
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex gap-2">
            ...
            <Button
              ...
              className={viewMode === "heatmap" ? "bg-purple-600 hover:bg-purple-700" : ""}
            >
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <Card className="border-purple-200 dark:border-purple-800">
            ...
            <CardContent>
              <p className="text-xs text-muted-foreground">ATLAS techniques tracked</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-purple-200 dark:border-purple-800">
            <CardHeader className="pb-3">
              ...
              <CardTitle className="text-3xl text-purple-600">{stats.coveragePercentage}%</CardTitle>
            </CardHeader>
            <CardContent>
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">Loading ATLAS coverage data...</p>
        </div>
      ) : viewMode === "heatmap" ? (
        renderHeatmapView()
      ) : coverage.length === 0 ? (
        <div className="text-center py-12 bg-card rounded-lg border border-border">
          ...
          <p className="text-muted-foreground font-medium">No ATLAS techniques mapped</p>
          <p className="text-sm text-muted-foreground mt-2">
            Start mapping ATLAS techniques to this operation to track coverage
          </p>
          <a
                  ...
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <a
                          href={`https://atlas.mitre.org/techniques/${mapping.technique.atlasId}`}
                          target="_blank"
                          rel="noopener noreferrer"
                      ...
                      {mapping.tactic && (
                        <p className="text-sm text-muted-foreground mt-1">
                          Tactic: {mapping.tactic.name} ({mapping.tactic.atlasId})
                        </p>
                      )}
                      {mapping.notes && (
