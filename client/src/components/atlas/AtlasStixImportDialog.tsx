...
export default function AtlasStixImportDialog() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to import ATLAS STIX bundle");
      }
      ...
      console.error("ATLAS import failed:", err.message);
    } finally {
      setImporting(false);
    }
  };
    ...
    >
      <DialogTrigger asChild>
        <Button>
          ...
          Import ATLAS STIX Data
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Import MITRE ATLAS STIX Bundle</DialogTitle>
          <DialogDescription>
            Upload a STIX 2.1 JSON bundle from MITRE ATLAS to populate the database
          </DialogDescription>
        </DialogHeader>
          ...
          {!importing && !stats && !error && (
            <div className="space-y-4">
              <div className="border-2 border-dashed border-border rounded-lg p-8 text-center hover:border-gray-400 transition-colors">
                ...
                <label htmlFor="atlas-file-upload" className="cursor-pointer">
                  <span className="text-sm font-medium text-foreground">
                    Click to upload ATLAS STIX JSON file
                  </span>
                  <input
                    ref={fileInputRef}
                    id="atlas-file-upload"
                    type="file"
                    accept=".json"
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  ...
                  >
                    MITRE ATLAS Navigator Data Repository
                  </a>
                </p>
              </div>
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                <h4 className="text-sm font-medium text-purple-900 mb-2">
                  Download ATLAS STIX Data
                </h4>
                <p className="text-xs text-purple-700 mb-3">
                  Get the latest ATLAS STIX bundle:
                </p>
                <code className="block text-xs bg-purple-100 p-2 rounded mb-2 text-purple-900">
                  wget https://raw.githubusercontent.com/mitre-atlas/atlas-navigator-data/main/dist/stix-atlas.json
                </code>
          ...
          {importing && (
            <div className="space-y-4">
              <div className="text-center">
                ...
                <p className="text-sm font-medium">Importing ATLAS STIX data...</p>
