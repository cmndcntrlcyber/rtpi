...
import AtlasTechniqueDetailDialog from "./AtlasTechniqueDetailDialog";
...
export default function AtlasTechniquesTable() {
  ...
  const fetchTechniques = async () => {
    ...
    try {
      ...
      const url = `/api/v1/atlas/techniques${params.toString() ? `?${params.toString()}` : ""}`;
      ...
      console.error("Failed to fetch ATLAS techniques:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const filtered = techniques.filter((t) => {
      ...
      const matchesSub = t.subtechniques?.some(
        (sub) =>
          sub.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          sub.atlasId.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleViewDetails = (technique: AtlasTechnique) => {
    setSelectedTechnique(technique);
    setDetailDialogOpen(true);
  };

  const handleExportToNavigator = () => {
    const flatTechniques: AtlasTechnique[] = [];
    ...
    exportToAtlasNavigator(flatTechniques, {
      layerName: `RTPI ATLAS${searchTerm ? ` - Search: ${searchTerm}` : ""}`,
      description: `Exported from RTPI on ${new Date().toLocaleDateString()}. Contains ${flatTechniques.length} techniques.`,
    });
  };

  const renderTechniqueRow = (technique: AtlasTechnique, isSubtechnique = false): JSX.Element => {
    const hasSubtechniques = !isSubtechnique && technique.subtechniques && technique.subtechniques.length > 0;
    ...
    return (
      <React.Fragment key={technique.id}>
        <TableRow className={isSubtechnique ? "bg-secondary" : ""}>
          <TableCell className="font-mono font-medium">
            <div className="flex items-center gap-2">
              {hasSubtechniques && (
                <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => toggleExpanded(technique.id)}>
                  {isExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                </Button>
              )}
              {!hasSubtechniques && !isSubtechnique && <div className="w-6" />}
              {isSubtechnique && <div className="w-10" />}
              <span>{technique.atlasId}</span>
              {isSubtechnique && (
            ...
            <div className="flex flex-wrap gap-1">
              {technique.killChainPhases && technique.killChainPhases.length > 0 ? (
                technique.killChainPhases.slice(0, 2).map((phase) => (
                  <Badge key={phase} variant="outline" className="text-xs">{phase.replace(/-/g, " ")}</Badge>
                ))
              ) : (
                <span className="text-sm text-muted-foreground">N/A</span>
              )}
          ...
          <TableCell className="text-right">
            <div className="flex gap-1 justify-end">
              <Button size="sm" variant="ghost" onClick={() => handleViewDetails(technique)} title="View details">
                <Info className="h-4 w-4" />
              </Button>
              <Button
                size="sm"
                variant="ghost"
                onClick={() => window.open(`https://atlas.mitre.org/techniques/${technique.atlasId}`, "_blank")}
        ...
        {hasSubtechniques && isExpanded && technique.subtechniques!.map((sub) => (
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">Loading ATLAS techniques...</p>
      </div>
    );
  }
  ...
  if (techniques.length === 0) {
    return (
      <div className="text-center py-12 bg-card rounded-lg border border-border">
        <p className="text-muted-foreground font-medium">No ATLAS techniques loaded</p>
        <p className="text-sm text-muted-foreground mt-2">Import ATLAS STIX data to populate techniques</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex gap-2">
          <Button size="sm" variant={filter === "techniques" ? "default" : "outline"} onClick={() => setFilter("techniques")}>
            Techniques Only
          </Button>
          <Button size="sm" variant={filter === "subtechniques" ? "default" : "outline"} onClick={() => setFilter("subtechniques")}>
            Sub-techniques Only
          </Button>
        ...
        <Button size="sm" variant="outline" onClick={handleExportToNavigator} disabled={filteredTechniques.length === 0}>
      ...
      <div className="text-sm text-muted-foreground">
        Showing {filteredTechniques.length} {filter === "techniques" ? "parent techniques" : filter === "subtechniques" ? "sub-techniques" : "techniques"}
        {filter === "techniques" && filteredTechniques.reduce((acc, t) => acc + (t.subtechniques?.length || 0), 0) > 0 && (
          <span> with {filteredTechniques.reduce((acc, t) => acc + (t.subtechniques?.length || 0), 0)} sub-techniques</span>
        )}
      </div>
