...
## Step 3.9: Bug Bounty Program Import

### Purpose

Automate the creation of operations from bug bounty program scope files. Users provide a HackerOne program slug (or upload files directly), and the system fetches the program's scope CSV and Burp project config, parses them, creates an operation with targets, and optionally triggers the scan pipeline cascade.

### MVP Scope

HackerOne is the initial platform. The URL patterns are:
- `https://hackerone.com/teams/{program}/assets/download_burp_project_file.json`
- `https://hackerone.com/teams/{program}/assets/download_csv.csv`

Extensible to Bugcrowd, Intigriti, YesWeHack via platform-specific parsers later.

### Data Format: HackerOne CSV

Comma-delimited with the following columns:

| Column | Type | Example |
|--------|------|---------|
| `identifier` | string | `hackerone.com`, `*.example.com`, `192.168.1.0/24` |
| `asset_type` | enum | `Domain`, `URL`, `Wildcard`, `CIDR`, `Source code`, etc. |
| `instruction` | string | Free text testing guidance |
| `eligible_for_bounty` | boolean | `true` / `false` |
| `eligible_for_submission` | boolean | `true` / `false` |
| `availability_requirement` | enum | `none`, `low`, `medium`, `high` |
| `confidentiality_requirement` | enum | `none`, `low`, `medium`, `high` |
| `integrity_requirement` | enum | `none`, `low`, `medium`, `high` |
| `max_severity` | enum | `critical`, `high`, `medium`, `low`, `none` |
| `system_tags` | string | Comma-separated values |
| `created_at` | ISO 8601 | `2024-01-15T00:00:00Z` |
| `updated_at` | ISO 8601 | `2024-06-20T00:00:00Z` |

HackerOne asset types include: `URL`, `DOMAIN`, `WILDCARD`, `CIDR`, `SOURCE_CODE`, `EXECUTABLE`, `HARDWARE`, `GOOGLE_PLAY_APP_ID`, `APPLE_STORE_APP_ID`, `TESTFLIGHT`, `IPA`, `APK`, `WINDOWS_STORE_APP_ID`, `AI_MODEL`, `SMART_CONTRACT`, `OTHER`.

The CSV is the **primary** import source — it contains all asset types and CVSS environmental modifiers that the Burp file lacks.

### Data Format: Burp Project Config JSON

Standard Burp Suite project configuration with regex-based scope:
...
Each scope entry uses regex for `host`, `port`, and `file`. Common patterns:
- `^example\\.com$` → exact domain `example.com`
- `^.*\\.example\\.com$` → wildcard `*.example.com`

Only URL/Domain type assets appear in the Burp file (no CIDR, mobile apps, source code, etc.). This file is **secondary** for scope extraction but **primary** for Burp Suite container configuration forwarding.

### Implementation File

```
server/services/bug-bounty-import-service.ts (NEW)
```

### Interface

```typescript
interface BugBountyProgramSource {
  platform: 'hackerone';  // extensible: 'bugcrowd', 'intigriti', 'yeswehack'
  programSlug: string;
  programUrl?: string;
}

interface ParsedScopeEntry {
  identifier: string;
  assetType: string;                          // original HackerOne asset_type
...
interface ParsedBountyScope {
  programName: string;
  platform: string;
  inScope: ParsedScopeEntry[];
  outOfScope: ParsedScopeEntry[];
  burpProjectConfig?: any;                    // raw Burp JSON for storage
}

interface ImportOptions {
  operationName?: string;                     // override default "Bug Bounty: {programName}"
  autoActivate?: boolean;                     // trigger scan pipeline immediately
  authHeaders?: Record<string, string>;       // for authenticated HackerOne fetch
}

interface BugBountyImportResult {
  success: boolean;
  operationId: string;
  operationName: string;
  targetsCreated: number;
  scopeEntriesTotal: number;
  scopeEntriesSkipped: number;                // non-targetable asset types
...
class BugBountyImportService {
   ...
   * Constructs URLs from slug, fetches CSV + Burp JSON, parses, creates operation + targets.
   * Falls back gracefully if one file fails (CSV is required, Burp JSON is optional).
   */
  async importFromUrl(
    programSlug: string,
    platform: 'hackerone',
  ...
  /**
   * Import from directly uploaded files (no URL fetching).
   */
   ...
   * Returns the parsed scope for user review before committing.
   */
  async preview(
    files: { csv?: string; burpJson?: string }
  ): Promise<ParsedBountyScope>;

  /**
   * Parse HackerOne comma-delimited CSV into scope entries.
   */
   ...
   * ^.*\\.example\\.com$ → *.example.com
   */
  private regexHostToDomain(regexHost: string): string;

  /**
   * Map HackerOne asset_type to RTPI targetTypeEnum.
   * Returns null for non-targetable types (mobile apps, source code, etc.).
   */
  private mapAssetTypeToTargetType(
    h1AssetType: string
  ...
  /**
   * Create operation and targets from parsed scope data.
   * Populates metadata.bugBounty, metadata.applicationOverview, metadata.scopeData.
   * Stores Burp project config in metadata.burpProjectConfig.
   */
...
| HackerOne `asset_type` | RTPI `targetTypeEnum` | Creates Target? |
|------------------------|----------------------|-----------------|
| `Domain` | `domain` | Yes |
| `URL` | `url` | Yes |
| `Wildcard` | `domain` | Yes (strip `*.` prefix for value) |
| `CIDR` | `network` | Yes |
| `Source code` | — | No (metadata only) |
| `Executable` | — | No (metadata only) |
| `Hardware/IoT` | — | No (metadata only) |
| `iOS: App Store` | — | No (metadata only) |
| `iOS: Testflight` | — | No (metadata only) |
| `iOS: .ipa` | — | No (metadata only) |
| `Android Play Store` | — | No (metadata only) |
| `Android: .apk` | — | No (metadata only) |
| `Windows: Microsoft Store` | — | No (metadata only) |
| `AI Model` | — | No (metadata only) |
| `Smart Contracts` | — | No (metadata only) |
| `Other` | — | No (metadata only) |

### Target Priority from max_severity

| `max_severity` | Target `priority` |
|----------------|-------------------|
| `critical` | 1 |
| `high` | 2 |
| `medium` | 3 |
...
interface BugBountyOperationMetadata {
  bugBounty: {
    platform: 'hackerone';
    programSlug: string;
    programUrl: string;                       // e.g., "https://hackerone.com/doordash"
    ...
    nonTargetableAssets: Array<{              // mobile apps, source code, etc.
      identifier: string;
      assetType: string;
      eligibleForBounty: boolean;
      maxSeverity: string;
  ...
  // Populated for compatibility with existing extractScopeTargets()
  applicationOverview: {
    inScopeDomains: string;                   // newline-separated domains
  };
  scopeData: {
    assetUrlInScope: string;                  // newline-separated URLs
    assetUrlOutScope: string;                 // newline-separated out-of-scope
  };

  // Raw Burp project config for Burp Suite container forwarding
  burpProjectConfig?: object;
}
```

```typescript
// Stored in target.metadata per target
interface BugBountyTargetMetadata {
  ...
  originalAssetType: string;                  // HackerOne asset_type before mapping
}
```
...
```
server/api/v1/bug-bounty-import.ts (NEW)
```

**Route prefix:** `/api/v1/bug-bounty-import`

| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/from-url` | Fetch scope from HackerOne by program slug |
| `POST` | `/upload` | Upload CSV and/or Burp JSON files directly |
| `POST` | `/preview` | Parse files without creating (dry run) |
...
{
  platform: 'hackerone',
  programSlug: string,            // e.g., "doordash"
  operationName?: string,         // override default name
  autoActivate?: boolean,         // default: false
  authHeaders?: Record<string, string>  // optional auth for private programs
}
...
Multipart form data via multer (memory storage, 50MB limit):
- `csvFile`: CSV file (optional if burpJsonFile provided)
- `burpJsonFile`: Burp project config JSON file (optional)
- `platform`: string field (default: `'hackerone'`)
- `programSlug`: string field (optional, for metadata)
- `operationName`: string field (optional)
- `autoActivate`: string field (`'true'`/`'false'`)

#### POST `/preview`

Same inputs as `/upload` but returns parsed scope without creating any records:
...
```
client/src/components/operations/BugBountyImportCard.tsx (NEW)
```

State machine UI following the `ScanImportCard.tsx` pattern:

```
idle → input → previewing → preview → importing → success
                                                 → error
```

**Input Mode Selector:** Toggle between "Enter Program URL" and "Upload Files"

**URL Mode:**
- Text input for HackerOne program slug (with helper text: `https://hackerone.com/teams/{slug}/...`)
- Optional text input for operation name override
- Checkbox: "Auto-activate operation (start scan pipeline immediately)"
- "Preview Scope" button → calls `POST /preview`

**Upload Mode:**
- Drag-and-drop zone accepting `.csv` and `.json` files (reuses ScanImportCard pattern)
- Shows selected file(s) with type badges (CSV, Burp JSON)
- Same options as URL mode

**Preview State:**
- Table with columns: Identifier | Asset Type | Target Type | Bounty Eligible | Max Severity
- Color coding: green for targetable assets, gray for metadata-only
- Collapsible out-of-scope section
- Summary line: "X targets will be created, Y assets stored as metadata only"
- "Create Operation" and "Cancel" buttons

**Success State:**
- Green card: operation name, targets created count, scope entries total
- "Go to Operation" button (navigates to `/operations`)
- "Import Another" button

**Placement in UI:**
- Operations page: "Import Bug Bounty" button alongside "+ New Operation"
- Rendered as a dialog/modal when triggered

### Integration with Existing Automation

1. **Operation lifecycle compatibility:** The service populates `metadata.applicationOverview.inScopeDomains` and `metadata.scopeData.assetUrlInScope` with parsed scope domains/URLs. This means the existing `extractScopeTargets()` in `operation-lifecycle-automation.ts` works without modification when the operation is activated.

2. **Two activation paths:**
   - **Immediate** (`autoActivate=true`): Import service sets `status: 'active'` and calls `operationLifecycleAutomation.handleOperationActivated()`. Target deduplication prevents duplicates since targets are already created. BBOT scan pipeline starts immediately.
   - **Deferred** (`autoActivate=false`, default): Operation created in `'planning'` status. User reviews, adjusts targets, then manually activates via existing UI.

3. **Burp Suite forwarding:** Raw Burp project config stored in `operation.metadata.burpProjectConfig`. When the user later activates Burp Suite scanning (via `burp-activation-service.ts`), the system reads this config and passes it to the Burp container as a mounted project configuration file.

4. **Target tagging:** All targets created from bug bounty import are tagged with `['auto-created', 'bug-bounty', platform]` (e.g., `['auto-created', 'bug-bounty', 'hackerone']`).

### Authentication Note

HackerOne scope downloads may require authentication for private programs. The MVP supports:
- **Public programs:** No auth needed, direct fetch works
- **Private programs:** User provides `authHeaders` (e.g., HackerOne session cookie or API token) in the request body. These are passed through to the fetch call and are NOT stored.
- **Fallback:** If URL fetch fails (401/403), the endpoint returns an error suggesting file upload as an alternative. Users can manually download the files from HackerOne and upload them.
...
server/services/bug-bounty-import-service.ts
server/api/v1/automation-pipeline.ts
server/api/v1/bug-bounty-import.ts
client/src/components/operations/BugBountyImportCard.tsx
migrations/0008_phase3_target_auto_creation.sql
```
...
server/index.ts                                     (register automation-pipeline + bug-bounty-import routes)
client/src/pages/Operations.tsx                     (add "Import Bug Bounty" button)
```
...
| 11 | Bug bounty import service + CSV/Burp parsers | 3-4 hours | Step 6 |
| 12 | Bug bounty import API endpoints | 1-2 hours | Step 11 |
| 13 | Bug bounty import UI component + Operations page integration | 2-3 hours | Step 12 |
...
- [ ] HackerOne CSV parsed correctly with comma delimiter, all 12 columns extracted
- [ ] Burp project config regex hosts converted to clean domain strings (`^example\\.com$` → `example.com`)
- [ ] Bug bounty import creates operation with `metadata.bugBounty` containing full scope data
- [ ] Targets created for Domain/URL/Wildcard/CIDR assets with bounty metadata (eligibility, max severity, environmental modifiers)
- [ ] Non-targetable assets (mobile apps, source code, AI models, etc.) stored in operation metadata only — no targets created
- [ ] Preview endpoint returns parsed scope without creating any database records
- [ ] Auto-activate option triggers existing scan pipeline cascade (BBOT → targets → nmap → nuclei)
- [ ] Burp project config stored in `operation.metadata.burpProjectConfig` for later Burp Suite container forwarding
...
- **Bug bounty program import** from HackerOne scope files (CSV + Burp JSON) with automatic operation and target creation
