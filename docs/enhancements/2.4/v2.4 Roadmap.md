# Meta-Instruction
Under each header are the things that need to be fixed or features that need to be created, as well as the conditions for success. This document serves as scaffolding for the complete plan which will be detailed by Claude in preparation. All implementation will document every step completed and follow a rigorous checklist, to prevent loss of development situational awareness.

If an Agent does not yet exist, create it and develop develop its necessary capabilities and workflow application routes. . 

Reporting Agents are not presented through the Operational UI, all Reporting Agents will be managed through the Administration page and deliver all output through the Operations Manager Agent.

---

## 📊 Implementation Status

**Overall Progress:** 90% COMPLETE (4.5 of 5 phases)
**Last Updated:** 2026-03-16
**Detailed Tracker:** [v2.4-IMPLEMENTATION-STATUS.md](./v2.4-IMPLEMENTATION-STATUS.md)

| Component | Status | Completion |
|-----------|--------|------------|
| Operations Manager Foundation | ✅ COMPLETE | 100% |
| Research Agent - Tavily Integration | ✅ COMPLETE | 100% |
| Maldev Agent - Custom Exploit Dev | ⚠️ PARTIAL | 70% |
| Maldev Tool Integration (v2.4.4) | ⚠️ PARTIAL | 70% |
| ATT&CK R&D Workflow Automation | ✅ COMPLETE | 100% |
| Dual-Agency Autonomous System | ⚠️ IN PROGRESS | 75% |

**Phase 3 COMPLETE (2026-03-16).** Phase 4: Review Agent, Operation Lead Agent, Technical Reviewer Agent, and Autonomous Operation Orchestrator implemented. Remaining: Sysreptor integration, Admin UI for reporter management.

# Troubleshooting
- Review the images within the ts-img folder, "[issue-name]/image"
    - Utilize all agents and tools until the issue is completely repaired

# v2.4 Objective (Agentic Autonomy)
I'd like to create a plan dual-agency-system for the RTPI platform,  Identify were scaffolding for this exists, what scaffolding would be necessary, and how to bridge the gaps so that an operation can begin and finsih completely autonomously. 

Please ask any clarifying questions necessary.

# Operations

## Types
A Continuous Operation may simply be a contuous reconnaissance effort of scannign once per-day

an operation may last up to one week of continuous testing and retesting

An Operation is complete when a human confirms that the final report product is acceptible. 

# Agentic Systems
## System-1 (Autonomous Operation System): [Triggered when an Operation is approved and Confirmed by a Human] 
1. Operation Manager Agent - Sends operation number and details to review agent

2. Review Agent - Parses Operation data, Determines what tasks need to be created,sends task information to task agent

3. Task Agent -  Reviews task, determines how to delegate tasks 
- [Pass to Technical Operation System]

- [Receive from Technical Report Writer Agent]

4. QA Agent - Ensures objectives have been complete, reporting grammar is clear, concise, and free of spelling mistakes or gramatical errors. 

5. Reporting Agent - 

## System 2 - ( Technical Operation System)
[Receive from Autonomous Task Agent]

1. Operation Lead Agent - Utilizies rtpi-x tool containers

2. Technical Operator [Offsec R&D Agent] -  Utilizies rtpi-x tool containers to accomplish operational objectives
3. Technical Reviewer [Offsec R&D Agent] - Utilizes rtpi-x tool containers to validate findings and confirms 
    - includes a contrarian objective, seeking to debase the validity of a finding.
    - WILL CONCEDE CONTRARION OBJECTIVE AFTER A FINDING IS FULLY VALIDATED AS A TRUE POSITIVE.

4. Technical Report Writer Agent - Utilizes internal reporting for each task, utilizes Sysreptor API and LLM inference to write final report

[Pass to Autonomous Operation QA Agent] (Up to 3x Loop for optimal reporting)

---

# Enhancement Specifications

## v2.4.1 - Research Agent Tavily Integration **[DONE]**
**File:** [v2.4.1-research-agent-tavily-integration.md](v2.4.1-research-agent-tavily-integration.md)

Deep vulnerability research via Tavily MCP when no known Metasploit module exists. The research-agent performs CVE discovery, PoC extraction, and exploit chain analysis, then routes findings to the maldev-agent for custom exploit development.

**Implementation:** `server/services/agents/research-agent.ts` — 3-phase pipeline (CVE Discovery, Exploit Intelligence, Attack Methodology). Auto-delegated from `agent-workflow-orchestrator.ts` when no MSF modules found in attack tree. Routes `VulnerabilityResearchPackage` to maldev-agent via message bus.

## v2.4.2 - Maldev Agent Custom Exploit Development **[PARTIAL - 50%]**
**File:** [v2.4.2-maldev-agent-custom-exploit-development.md](v2.4.2-maldev-agent-custom-exploit-development.md)

Custom exploit crafting (Metasploit modules, Nuclei templates, reverse shell payloads) from vulnerability research. Targets Metasploit multi/handler for reverse shell delivery on network targets.

**Implementation:** `server/services/agents/maldev-agent.ts` — Receives research packages, generates MSF modules (AI-powered via Ollama + fallback template), Nuclei detection templates (stored in DB), and payload configurations. Custom modules loaded via `metasploitExecutor.loadCustomModule()`.

**⚠️ Critical Gap:** Agent uses AI-only code generation but never executes the 44 reverse-engineering tools installed in its container (Ghidra, ROPgadget, pwntools, TamperETW, etc.). See v2.4.4 below.

## v2.4.3 - ATT&CK R&D Workflow Automation **[IN PROGRESS - 85%]**
**File:** [v2.4.2-attack-research-development-workflows.md](v2.4.2-attack-research-development-workflows.md)

Complete automated end-to-end workflow from vulnerability discovery through research, POC development, tool production, testing, and deployment into ATT&CK-based operational workflows.

**Status:** 6 of 7 components implemented (2026-03-16). Only ATT&CK Workflow Builder UI remains.
1. ✅ R&D Experiment Orchestrator (`server/services/rd-experiment-orchestrator.ts`)
2. ✅ Artifact Storage System (`rdArtifacts` table in `shared/schema.ts`, `rdArtifactId` on `toolRegistry`)
3. ✅ Tool Promotion Service (`server/services/rd-tool-promotion.ts`)
4. ✅ POC Development Agent (`server/services/agents/poc-development-agent.ts`)
5. ✅ Nuclei Template Agent (`server/services/agents/nuclei-template-agent.ts`)
6. ✅ Tool Testing Feedback Loop (`server/services/rd-feedback-loop.ts` + `tool-tester.ts` EventEmitter)
7. ⚠️ ATT&CK Workflow Builder — backend service complete (`server/services/attack-workflow-builder.ts`), UI integration pending

**API Endpoints Wired:**
- `POST /api/v1/offsec-rd/experiments/:id/execute` — orchestrator execution
- `POST /api/v1/offsec-rd/projects/:projectId/execute` — batch project execution
- `POST /api/v1/offsec-rd/artifacts/:artifactId/promote` — tool promotion
- `POST /api/v1/attack-flows/build-workflow` — ATT&CK workflow builder
- `GET /api/v1/attack-flows/tools-for-technique/:attackId` — tool-technique mapping

**Remaining Effort:** ~12-16 hours (ATT&CK Workflow Builder UI components)

## v2.4.4 - Maldev Agent Tool Integration **[NEW - NOT STARTED]**
**Status:** Required enhancement discovered during v2.4 implementation review.

Integrate the 44 reverse-engineering, ROP development, and evasion tools in the rtpi-maldev-agent container with the maldev-agent workflow.

**Current State:** Container has tools but agent never uses them:
- **Reverse Engineering:** Ghidra MCP, radare2, x64dbg, WinDbg, binary analysis
- **ROP Development:** ROPgadget, RustChain, ropium, xgadget
- **Shellcode/Evasion:** pwntools, TamperETW, SHAPESHIFTER, SysWhispers
- **Validation:** unicorn emulator, capstone, keystone

**Required Implementation:**
1. Create `server/services/agents/maldev-tool-executor.ts` — Wrapper for container tool execution
2. Enhance maldev-agent.ts with 5-phase pipeline:
   - Phase 1: Binary Analysis (Ghidra/radare2)
   - Phase 2: ROP Chain Development (ROPgadget/RustChain)
   - Phase 3: Shellcode Generation (pwntools)
   - Phase 4: AI-Assisted Code Generation (with tool insights)
   - Phase 5: Validation (unicorn emulation)
3. Register maldev tools in Tool Registry
4. Add comprehensive testing

**Estimated Effort:** 10-14 hours  
**Impact:** HIGH - Transforms AI-only generation into tool-assisted sophisticated exploit development

## Fixes Applied (Pre-v2.4)

The following bugs were fixed as prerequisites for the v2.4 workflow enhancements:

| Fix | File | Description |
|-----|------|-------------|
| Metasploit module path double-prefix | `agent-workflow-orchestrator.ts`, `metasploit-executor.ts` | AI prompt example caused `auxiliary/auxiliary/...` paths; added defensive sanitization |
| Report truncation at Low Severity | `agent-workflow-orchestrator.ts`, `ollama-ai-client.ts` | Technical Writer had 2048 max_tokens; increased to 16384 with truncation detection |
| Web Hacker Agent `cmd.join` crash | `web-hacker-agent.ts` | Docker exec called with wrong signature (string vs string[]) |
| Vulnerability Reporter Agent DB insert | `vulnerability-reporter-agent.ts` | Insert used non-existent columns; fixed to use `type`/`config` per schema |
| Session timeout too short | `server/auth/session.ts` | Cookie maxAge increased from 30 minutes to 24 hours |
| OffSec containers not starting | `docker-compose.yml` | Removed `profiles: offsec-agents` gate from all 7 containers |