 ...
 * Run with: npx tsx server/scripts/tools/migrate-existing-tools.ts
 */
...
const log = createLogger("migrate-existing-tools");
...
async function migrateTools() {
  log.info('🔄 Starting tool migration to new framework...\n');
  ...
  for (const tool of tools) {
    try {
      ...
      log.info(`✅ ${tool.name} registered successfully with ID: ${toolId}\n`);
      successCount++;
    } catch (error: any) {
      log.error(`❌ Failed to register ${tool.name}:`, error.message);

      // If tool already exists, that's okay
      if (error.message.includes('already exists')) {
        log.info(`ℹ️  ${tool.name} already registered, skipping...\n`);
        successCount++;
      } else {
        failCount++;
