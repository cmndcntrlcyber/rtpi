...
function generateReportContent(reportData: any, _template?: any): string {
  ...
  switch (type) {
    ...
    case "surface_assessment":
      content += generateSurfaceAssessmentReport(reportData);
      break;
    default:
...
function generateSurfaceAssessmentReport(data: any): string {
  const summary = data.content?.summary || {};
  return `## Surface Assessment Report
...
| Total Vulnerabilities | ${summary.totalVulnerabilities || 0} |

### Severity Distribution
- **Critical:** ${summary.severityDistribution?.critical || 0}
- **High:** ${summary.severityDistribution?.high || 0}
- **Medium:** ${summary.severityDistribution?.medium || 0}
- **Low:** ${summary.severityDistribution?.low || 0}
- **Informational:** ${summary.severityDistribution?.informational || 0}

## Framework Compliance Mapping

Framework analysis will be populated by the Framework Security Agent.

## Remediation Roadmap

Remediation priorities will be generated based on framework analysis.

---

*Surface Assessment Report Generated by RTPI on ${new Date().toLocaleString()}*
`;
}
